...
const stats: Stat[] = [
  { label: 'Series curated', value: '24' },
  { label: 'Prints sold', value: '320+' },
  { label: 'Global features', value: '18' },
];
    ...
    >
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-24 px-6 pb-24 pt-10 sm:px-10">
        <div className="sticky top-6 z-40">
          <nav
            aria-label="Main"
            className="flex items-center justify-between rounded-full border border-white/10 bg-white/10 px-6 py-3 backdrop-blur-xl"
          >
            <span className="text-sm font-semibold tracking-[0.35em] text-white/70">
              MYPHOTO
            </span>
            <ul className="hidden items-center gap-6 text-sm font-medium text-white/70 md:flex">
              {navItems.map((item) => (
                <li key={item.label}>
                  <Link
                    href={item.href}
                    className="group relative overflow-hidden rounded-full px-3 py-1 transition-colors duration-300 hover:text-white"
                  >
            ...
            <button
              type="button"
              className="group relative overflow-hidden rounded-full border border-white/30 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-white transition duration-300 hover:border-white hover:text-[#ff1f8c]"
            >
        ...
        <header className="grid items-center gap-16 md:grid-cols-[1.2fr_1fr]">
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
              <Link
                href="#work"
                className="group relative w-full overflow-hidden rounded-full bg-[#ff1f8c] px-7 py-3 text-center text-sm font-semibold uppercase tracking-[0.25em] text-[#0f1115] transition-transform duration-500 hover:-translate-y-0.5 sm:w-auto"
              >
              ...
              <Link
                href="#contact"
                className="group relative w-full overflow-hidden rounded-full border border-white/30 px-7 py-3 text-center text-sm font-semibold uppercase tracking-[0.25em] text-white transition duration-500 hover:border-[#00e5ff] hover:text-[#00e5ff] sm:w-auto"
              >
            ...
            <div className="grid grid-cols-3 gap-3">
              {stats.map((stat) => (
                ...
                >
                  ...
                  <span className="mt-1 block text-xs uppercase tracking-[0.25em] text-white/40">
                    {stat.label}
                  </span>
                </div>
              ))}
        ...
        <section id="work" className="flex flex-col gap-10">
          <div className="flex flex-col gap-3">
            ...
            <h2 className="text-3xl font-semibold sm:text-4xl">
              Masonry cards with glass glow + kinetic hover
            </h2>
